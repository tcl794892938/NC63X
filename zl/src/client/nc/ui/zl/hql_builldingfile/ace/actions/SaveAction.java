...
public class SaveAction extends nc.ui.pubapp.uif2app.actions.DifferentVOSaveAction {
	...
	public void doAction(ActionEvent e) throws Exception {
		...
		AggBuildingfileVO aggvo = (AggBuildingfileVO)billform.getValue();
		...
		if(getModel().getUiState()==UIState.EDIT){
			String sql2 = "select code from zl_buildingfile where pk_buildingfile ='"+aggvo.getParentVO().getPk_buildingfile()+"'";
		...
		Object obj= aggvo.getParentVO().getAttributeValue("pk_buildingfile");
		String sql="select count(1) from zl_buildingfile where nvl(dr,0)=0 and code='"+code+"' "
				+ "and pk_buildingfile<>'"+obj+"' and pk_projectid='"+proj+"'";
		
		Integer it=(Integer)iQ.executeQuery(sql, new ColumnProcessor());
		if(it>0){
			MessageDialog.showHintDlg(billform, "提示", "该项目下楼栋编码已经存在！");
			return ;
		}*/
		...
		if(getModel().getUiState()==UIState.ADD){
			...
			String sql="select max(code) from zl_buildingfile where nvl(dr,0)=0 and pk_projectid = '"+aggvo.getParentVO().getPk_projectid()+"'"+
