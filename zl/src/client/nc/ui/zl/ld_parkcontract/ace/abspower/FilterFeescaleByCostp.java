package nc.ui.zl.ld_parkcontract.ace.abspower;
...
public class FilterFeescaleByCostp extends AbstractLinkageColumnListener {

	private String orgField = null;
	private String targetField = null;
	private String filterField = null;

	public FilterFeescaleByCostp(QueryConditionDLGDelegator dlg, String orgField, String basedocField) {
		super(dlg);
		this.orgField = orgField;
		this.targetField = basedocField;
	}

	public FilterFeescaleByCostp(QueryConditionDLGDelegator dlg, String orgField, String basedocField, String filterField) {
	...
	public void processLinkageLogic(List<IFieldValueElement> fatherValues, IFilterEditor editor) {
		...
		if(refPane.getRefModel().getPkFieldCode().equals("pk_feescale")){
			filterField="***";
		}else{
			filterField=null;
		}
		...
		if (diffValues.size() >=0) {
			if(diffValues.size()==0&&!StringUtils.isEmpty(filterField)){
				refPane.getRefModel().addWherePart(" and chargeitem ='' ");
			...
			if (!StringUtils.isEmpty(filterField) && diffValues.size()>0) {
				...
				refPane.getRefModel().addWherePart(" and chargeitem ='"+pk_orgs[0]+"' ");
