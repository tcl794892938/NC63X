...
public class BusinessFastQuery {
	...
	public IQueryScheme doQuery(IQueryScheme queryScheme) throws Exception {
		...
		String usersql = "select * from zl_sales where nvl(dr,0)=0 and username=(select pk_base_doc from sm_user where cuserid='"
				+ user + "') ";
		SalesVO vo = (SalesVO) iQ.executeQuery(usersql, new BeanProcessor(
				SalesVO.class));
		if (vo != null) {
			wherenew += " and zl_business.salesman='" + vo.getUsername() + "'";
		...
		String get_pk_pro = "select c.pk_project from zl_projectcontrol c where nvl(c.dr,0)=0 and c.pk_projectcontrol in "
				...
				+ "b.usercode=(select s.pk_base_doc from sm_user s where nvl(s.dr,0)=0 and s.cuserid='"
				+ AppContext.getInstance().getPkUser() + "'))";
		List<Object> pk_pro = (List<Object>) iQ.executeQuery(get_pk_pro,
		...
		if (pros != "") {
			wherenew += " and exists (select b.* from zl_business_b b "
					+ "where nvl(b.dr,0)=0 and b.pk_bussiness=zl_business.pk_bussiness and b.procode in ("
					+ pros + "))";
			obj2.setWhere(wherenew);
		}
