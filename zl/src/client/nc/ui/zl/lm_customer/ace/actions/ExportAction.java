...
public class ExportAction extends NCAction {
	...
	public void doAction(ActionEvent arg0) throws Exception {
		...
		map.put(1, card.getHeadItem("customername").getName());
		map.put(2, card.getHeadItem("customertype").getName());
		map.put(3, card.getHeadItem("customerlxfs").getName());
		map.put(4, card.getHeadItem("customeraddress").getName());
		map.put(5, card.getHeadItem("zygw").getName());
		map.put(6, card.getHeadItem("sfzh").getName());
		map.put(7, card.getHeadItem("yyzz").getName());
		map.put(8, card.getHeadItem("fzkh").getName());
		map.put(9, card.getBodyItem("pk_project").getName());
		
		String[] headColsCN = new String[] { map.get(0), map.get(1), map.get(2), map.get(3),
				map.get(4), map.get(5), map.get(6), map.get(7), map.get(8) };
		String[] headColsCN2=new String[] {"表头行号",map.get(9)};

		doExport("客户信息中心", null, headColsCN,headColsCN2, null);
	}
