...
public class RefParkAction extends AbstractReferenceAction{

	public RefParkAction() {
		super();
		this.setCode("refaction");
		this.setBtnName("参照车位合同");
	}
	...
	public void doAction(ActionEvent arg0) throws Exception {
		...
		if (PfUtilClient.isCloseOK()) {
			...
			AggParkcontractVO[] vos= (AggParkcontractVO[])PfUtilClient.getRetOldVos();
			 ...
			 for(int i=0;i<vos.length;i++){
				...
				// vo.setPk_poject(vos[i].getParentVO().getPk_project());
				 ...
				 vo.setVbillstatus(-1);
				 vo.setVsrcbid(vos[i].getParentVO().getPk_parkcontract());
				 ...
				 String maxversion="select max(version) from zl_carconedit where nvl(dr,0)=0 and vsrcbid='"+vos[i].getParentVO().getPk_parkcontract()+"'";
				 ...
				 //基本信息
				 ParkcontractBVO[] cbvo = (ParkcontractBVO[]) vos[i].getTableVO("pk_parkcontract_b");
				 ...
				 for(int j=0;j<cbvo.length;j++){
					 ...
					 bvo[j].setPk_building(cbvo[j].getParkarea());
					 bvo[j].setPk_house(cbvo[j].getParknum());
				 ...
				 //费用信息
				 //查询费用信息
				 String sql1="select * from zl_parkcontract_c where nvl(dr,0)=0 and pk_parkcontract in(select pk_parkcontract from zl_parkcontract where vbillstatus=1 and version=-1 and nvl(dr,0)=0 ) order by to_number(rowno)";
				 List<ParkcontractCVO> carCList = (List<ParkcontractCVO>)iQ.executeQuery(sql1, new BeanListProcessor(ParkcontractCVO.class));
				 List<ParkcontractCVO> carCList1 = new ArrayList<ParkcontractCVO>();
				 if(carCList.size()>0){
					 ParkcontractCVO cavo = null;
					for(int a=0;a<carCList.size();a++){
						if(vos[i].getParentVO().getPk_parkcontract().equals(carCList.get(a).getPk_parkcontract())){
							cavo = new ParkcontractCVO();
							cavo = carCList.get(a);
							carCList1.add(cavo);
						}
					}
					 ...
					 for(int j=0;j<carCList1.size();j++){
						 ...
						 cvo[j].setPk_building(carCList1.get(j).getParkarea());
						 cvo[j].setPk_house(carCList1.get(j).getParknum());
						 for(int m=0;m<cbvo.length;m++){
							 if(cbvo[m].getParkarea().equals(carCList1.get(i).getParkarea())&&cbvo[m].getParknum().equals(carCList1.get(i).getParknum())){
								 cvo[j].setPk_plate(cbvo[m].getPlatenum());
							 }
						 }
				...
				//财务分摊
				//查询财务分摊
				String sql2= "select * from zl_parkcontract_f where nvl(dr,0)=0 and pk_parkcontract in(select pk_parkcontract from zl_parkcontract where vbillstatus=1 and version=-1 and nvl(dr,0)=0 ) order by to_number(rowno)";
				List<ParkcontractFVO> carFList = (List<ParkcontractFVO>)iQ.executeQuery(sql2, new BeanListProcessor(ParkcontractFVO.class));
				List<ParkcontractFVO> carFList1 = new ArrayList<ParkcontractFVO>();
				if(carFList.size()>0){
					ParkcontractFVO cfvo = null;
					for(int a=0;a<carFList.size();a++){
						if(vos[i].getParentVO().getPk_parkcontract().equals(carFList.get(a).getPk_parkcontract())){
							cfvo = new ParkcontractFVO();
							cfvo = carFList.get(a);
							carFList1.add(cfvo);
						}
					}
				    ...
				    for(int j=0;j<carFList1.size();j++){
						 ...
						 fvo[j].setPk_building(carFList1.get(j).getParkarea());
						 fvo[j].setPk_house(carFList1.get(j).getParknum());
						 for(int m=0;m<cbvo.length;m++){
							 if(cbvo[m].getParkarea().equals(carFList1.get(j).getParkarea())&&cbvo[m].getParknum().equals(carFList1.get(j).getParknum())){
								 fvo[j].setPk_plate(cbvo[m].getPlatenum());
							 }
						 }
