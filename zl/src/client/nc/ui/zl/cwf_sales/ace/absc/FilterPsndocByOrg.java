...
public class FilterPsndocByOrg extends AbstractLinkageColumnListener {
	
	private String orgField = null;
	private String targetField = null;
	private String filterField = null;
	
	public FilterPsndocByOrg(QueryConditionDLGDelegator dlg, String orgField, String basedocField) {
		super(dlg);
		this.orgField = orgField;
		this.targetField = basedocField;
	}

	public FilterPsndocByOrg(QueryConditionDLGDelegator dlg, String orgField, String basedocField, String filterField) {
	...
	public void processLinkageLogic(List<IFieldValueElement> fatherValues, IFilterEditor editor) {
		...
		if(refPane.getRefModel().getPkFieldCode().equals("bd_psndoc.pk_psndoc")){
			filterField="***";
		}else{
			filterField=null;
		}
		...
		if (diffValues.size() >=0) {
			...
			if (!StringUtils.isEmpty(filterField) && diffValues.size()>0) {
				...
				//refmodel.addWherePart(" and pk_org ='"+pk_orgs[0]+"'");
				//refmodel.setClassWherePart(refmodel.getClassWherePart()+" and pk_org ='"+pk_orgs[0]+"' ");
				//refmodel.addBlurWherePart(" and pk_org ='"+pk_orgs[0]+"' ");
