...
public class MusFilterProjectByOrg extends AbstractLinkageColumnListener {

	private String orgField = null;
	private String targetField = null;
	private String filterField = null;

	public MusFilterProjectByOrg(QueryConditionDLGDelegator dlg, String orgField, String basedocField) {
		super(dlg);
		this.orgField = orgField;
		this.targetField = basedocField;
	}

	public MusFilterProjectByOrg(QueryConditionDLGDelegator dlg, String orgField, String basedocField, String filterField) {
			...
			if (!StringUtils.isEmpty(filterField) && diffValues.size()>0) {
				...
				try {
					...
					String count="select count(*) from zl_projectcontrol_b where nvl(dr,0)=0 and pk_projectcontrol " +
							...
							"p.pk_org in ("+p+")) and usercode = (select s.pk_base_doc from sm_user s " +
