...
public class FilterCostpByOrg2 extends AbstractLinkageColumnListener {

	private String orgField = null;
	private String targetField = null;
	private String filterField = null;

	public FilterCostpByOrg2(QueryConditionDLGDelegator dlg, String orgField, String basedocField) {
		super(dlg);
		this.orgField = orgField;
		this.targetField = basedocField;
	}

	public FilterCostpByOrg2(QueryConditionDLGDelegator dlg, String orgField, String basedocField, String filterField) {
	...
	public void processLinkageLogic(List<IFieldValueElement> fatherValues, IFilterEditor editor) {
		...
		/*if (refPane.getRefModel().getVecData() == null || refPane.getRefModel().getVecData().size() == 0) {
			...
			Vector<Object> result=refPane.getRefModel().getQueryResultVO();
			Object[] re=result.toArray();
			Integer al=0;
			for(int i=0;i<re.length;i++){
				if(re[i].toString().startsWith("[05")||re[i].toString().startsWith("[06")){
					result.remove(i-al);
					al++;
				}
			}
			refPane.getRefModel().setSelectedData(result);
		}*/
		...
		if(refPane.getRefModel().getPkFieldCode().equals("pk_costproject")){
			filterField="***";
		}else{
			filterField=null;
		}
