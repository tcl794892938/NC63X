...
public class MusFilterCustomerByProject extends AbstractLinkageColumnListener{
	...
	public MusFilterCustomerByProject(QueryConditionDLGDelegator dlg, String orgField, String basedocField) {
		super(dlg);
		this.orgField = orgField;
		this.targetField = basedocField;
	}

	public MusFilterCustomerByProject(QueryConditionDLGDelegator dlg, String orgField, String basedocField, String filterField) {
	...
	public void processLinkageLogic(List<IFieldValueElement> fatherValues, IFilterEditor editor) {
		...
		if(refPane.getRefModel().getPkFieldCode().equals("pk_customer")){
			filterField="***";
		}else{
			filterField=null;
		}
		...
		if (diffValues.size() >= 0) {
			if(diffValues.size()==0&&!StringUtils.isEmpty(filterField)){
				refPane.getRefModel().addWherePart(" and pk_project ='' ");
			...
			if (!StringUtils.isEmpty(filterField) && diffValues.size()>0) {
				...
				refPane.getRefModel().addWherePart(" and exists (select b.* from zl_customer_zzxm b " +
						"where nvl(b.dr,0)=0 and b.pk_customer=zl_customer.pk_customer and b.pk_project in ("+p+")) ");
