...
public class MusFilterBuildingByProject extends AbstractLinkageColumnListener {

	private String orgField = null;
	private String targetField = null;
	private String filterField = null;

	public MusFilterBuildingByProject(QueryConditionDLGDelegator dlg, String orgField, String basedocField) {
		super(dlg);
		this.orgField = orgField;
		this.targetField = basedocField;
	}

	public MusFilterBuildingByProject(QueryConditionDLGDelegator dlg, String orgField, String basedocField, String filterField) {
	...
	public void processLinkageLogic(List<IFieldValueElement> fatherValues, IFilterEditor editor) {
		...
		if(refPane.getRefModel().getPkFieldCode().equals("pk_buildingfile")){
			filterField="***";
		}else{
			filterField=null;
		}
		...
		if (diffValues.size() >= 0) {
			...
			if (!StringUtils.isEmpty(filterField) && diffValues.size()>0) {
				...
				refPane.getRefModel().addWherePart(" and pk_projectid in("+p+") ");
