...
public class FilterFamilyfileByProject extends AbstractLinkageColumnListener {

	private String orgField = null;
	private String targetField = null;
	private String filterField = null;

	public FilterFamilyfileByProject(QueryConditionDLGDelegator dlg, String orgField, String basedocField) {
		super(dlg);
		this.orgField = orgField;
		this.targetField = basedocField;
	}

	public FilterFamilyfileByProject(QueryConditionDLGDelegator dlg, String orgField, String basedocField, String filterField) {
	...
	public void processLinkageLogic(List<IFieldValueElement> fatherValues, IFilterEditor editor) {
		...
		if(refPane.getRefModel().getPkFieldCode().equals("pk_familyfile")){
			filterField="***";
		}else{
			filterField=null;
		}
		...
		if (diffValues.size() >= 0) {
			if(diffValues.size()==0&&!StringUtils.isEmpty(filterField)){
				refPane.getRefModel().addWherePart(" and pk_projectid ='' ");
			...
			if (!StringUtils.isEmpty(filterField) && diffValues.size()>0) {
				...
				refPane.getRefModel().addWherePart(" and pk_projectid ='"+pk_orgs[0]+"' ");
